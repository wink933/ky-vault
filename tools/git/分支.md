...
# 8) VS Code 里怎么用分支（对应关系）

VS Code 左下角会显示当前分支名：

- 点击分支名 → Create new branch / Switch branch
    
- Source Control 里 commit/push/pull 都能点
    
- 冲突合并时有可视化按钮（Accept Current/Incoming/Both）
    

本质上 VS Code 只是把这些命令做成了 UI。
...
如果你想把“分支”真正用熟，我建议下一步我用一个最小练习把它串起来：  
**开分支 → 提交两次 → 推到 GitHub → 发 PR → 合并 → 删除分支**。  
你更想先学哪一种合并策略：**merge（更安全直观）**还是 **rebase（历史更干净）**？