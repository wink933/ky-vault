...
#### 基本结构
其基本结构如图 2.7 所示：$A$ 和 $B$ 为两个 $n$ 位操作数输入端，$C_{in}$ 为进位输入端，$ALUop$ 为操作控制信号，用于选择 ALU 执行的具体功能。 
**如果 ALU 支持 $k$ 种功能，则控制信号位数 $m \ge \lceil \log_2 k \rceil$**
>控制信号的一种状态就代表了一个功能



需要我为您生成相关的逻辑电路示意图，或者继续转写其他页面吗？
