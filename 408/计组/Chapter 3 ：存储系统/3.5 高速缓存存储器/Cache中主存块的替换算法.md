...
3. **最近最少使用（LRU）算法**：基于程序访问的局部性原理，优先替换最近最久未被访问的 Cache 行。其平均命中率通常高于 FIFO。**LRU** 算法是考查重点。
	...
	LRU 位的位数取决于组的路数：2 路组相联需 1 位 LRU 位，4 路组相联需 2 位 LRU 位。  
	假定采用 4 路组相联，5 个主存块 $\{1, 2, 3, 4, 5\}$ 映射到同一 Cache 组，访问序列为 $\{1, 2, 3, 4, 1, 2, 5, 1, 2, 3, 4, 5\}$，LRU 替换过程如图 3.22 所示。 
	...
	计数器的**更新规则**：  
	① 命中时，所命中行的**计数器清零**，**比其低的计数器加 1**，其余不变；
	② 未命中且有空闲行时，新装入的行的计数器置 0，其他非空闲行全加 1；
	③ 未命中且无空闲行时，替换**计数值最大**（本例中为 3）的行，新装入的行的计数器**置 0**，其余**全加 1**。
