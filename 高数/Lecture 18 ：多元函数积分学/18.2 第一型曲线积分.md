...
3. 对于光滑曲线 $L$，线密度为 $\rho(x, y, z)$，则计算该曲线对 $x$ 轴、$y$ 轴、$z$ 轴和原点 $O$ 的转动惯量 $I_x, I_y, I_z$ 和 $I_O$ 公式分别为
   $$I_x = \int_L (y^2 + z^2) \rho(x, y, z) \, \mathrm{d}s, I_y = \int_L (z^2 + x^2) \rho(x, y, z) \, \mathrm{d}s,$$

$$I_z = \int_L (x^2 + y^2) \rho(x, y, z) \, \mathrm{d}s, I_O = \int_L (x^2 + y^2 + z^2) \rho(x, y, z) \, \mathrm{d}s.$$