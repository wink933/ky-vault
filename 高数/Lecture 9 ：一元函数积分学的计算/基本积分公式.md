---
title: 基本积分公式
tags:
  - "#基本积分公式"
categories:
date: 2026-02-18
---
①  
$$  
\int x^k,dx=\frac{1}{k+1}x^{k+1}+C,\ k\ne-1;\quad  
\begin{cases}  
\int \dfrac{1}{x^2},dx=-\dfrac{1}{x}+C,\[6pt]  
\int \dfrac{1}{\sqrt{x}},dx=2\sqrt{x}+C.  
\end{cases}  
$$

②  
$$  
\int \frac{1}{x},dx=\ln|x|+C.  
$$  
$\rightarrow(\ln x)'=\frac{1}{x}$

③  
$$  
\int e^x,dx=e^x+C;\quad \int a^x,dx=\frac{a^x}{\ln a}+C,\ a>0\ \text{且}\ a\ne1.  
$$  
$\rightarrow【看不清】$

④  
$$  
\begin{cases}  
\int \sin x,dx=-\cos x+C,\[6pt]  
\int \cos x,dx=\sin x+C;  
\end{cases}  
\qquad  
\begin{cases}  
\int \tan x,dx=-\ln|\cos x|+C,\[6pt]  
\int \cot x,dx=\ln|\sin x|+C;  
\end{cases}  
$$

角度一：$(-\ln|\cos x|)'=\tan x\Rightarrow \int \tan x,dx=-\ln|\cos x|+C.$

角度二：$\int \tan x,dx=\int \dfrac{\sin x}{\cos x},dx=-\int \dfrac{1}{\cos x}(\cos x)',dx\ (凑微分)$

$u=\cos x$

$=-\int \dfrac{1}{u},du=-\ln|u|+C=-\ln|\cos x|+C.$

$$  
\int \frac{dx}{\cos x}=\int \sec x,dx=\ln|\sec x+\tan x|+C;\ \ ★  
$$  
$$  
\int \frac{dx}{\sin x}=\int \csc x,dx=\ln|\csc x-\cot x|+C;  
$$  
$$  
\begin{cases}  
\int \sec^2 x,dx=\tan x+C,\[6pt]  
\int \csc^2 x,dx=-\cot x+C;  
\end{cases}  
\qquad  
\begin{cases}  
\int \sec x\tan x,dx=\sec x+C,\[6pt]  
\int \csc x\cot x,dx=-\csc x+C.  
\end{cases}  
$$

⑤  
$$  
\begin{cases}  
\int \dfrac{1}{1+x^2},dx=\arctan x+C,\[8pt]  
\int \dfrac{1}{a^2+x^2},dx=\dfrac{1}{a}\arctan\dfrac{x}{a}+C\ (a>0).  
\end{cases}  
$$

⑥  
$$  
\begin{cases}  
\int \dfrac{1}{\sqrt{1-x^2}},dx=\arcsin x+C,\[8pt]  
\int \dfrac{1}{\sqrt{a^2-x^2}},dx=\arcsin\dfrac{x}{a}+C\ (a>0).  
\end{cases}  
$$

⑦  
$$  
\begin{cases}  
\int \dfrac{1}{\sqrt{x^2+a^2}},dx=\ln\bigl(x+\sqrt{x^2+a^2}\bigr)+C\ (\text{常见}a=1),\[10pt]  
\int \dfrac{1}{\sqrt{x^2-a^2}},dx=\ln\left|x+\sqrt{x^2-a^2}\right|+C\ (|x|>|a|).  
\end{cases}  
$$

⑧  
$$  
\int \frac{1}{x^2-a^2},dx=\frac{1}{2a}\ln\left|\frac{x-a}{x+a}\right|+C  
\left(\int \frac{1}{a^2-x^2},dx=\frac{1}{2a}\ln\left|\frac{x+a}{x-a}\right|+C\right).  
$$

⑨  
$$  
\int \sqrt{a^2-x^2},dx=\frac{a^2}{2}\arcsin\frac{x}{a}+\frac{x}{2}\sqrt{a^2-x^2}+C\ (a>|x|\ge0).  
$$

⑩  
$$  
\int \sin^2 x,dx=\frac{x}{2}-\frac{\sin 2x}{4}+C\ \left(\sin^2 x=\frac{1-\cos 2x}{2}\right);  
$$  
$\rightarrow$记住2次方即可

$$  
\int \cos^2 x,dx=\frac{x}{2}+\frac{\sin 2x}{4}+C\ \left(\cos^2 x=\frac{1+\cos 2x}{2}\right);  
$$  
$$  
\int \tan^2 x,dx=\tan x-x+C\ \left(\tan^2 x=\sec^2 x-1\right);  
$$  
$$  
\int \cot^2 x,dx=-\cot x-x+C\ \left(\cot^2 x=\csc^2 x-1\right).  
$$